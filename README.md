...
- すごろくのループ盤面（縦画面向けの縦長リング（既定5列×7行＝周回20マス）。グリッドのアスペクト比を保って中央配置し画面比に依らずマスが均等に並ぶ。外周マスを手番プレイヤーのコマが出目ぶん移動。コマは各プレイヤーのキャラの丸アイコン画像で描画（YOU＝選択キャラ、CPU＝人間と別のキャラをランダム選択、未配置は色＋YOU/CPU ラベルにフォールバック）。1周してゴール到達で勝ち）
