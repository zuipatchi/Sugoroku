...
- 円盤ルーレット（8分割・出目1〜8。ボタンを長押し中は回転し、離すと減速して止まった出目で移動マス数を決定。各セクターにキャラアイコンを表示。CPU の番は同じ円盤が自動で回る）
- CPU 対戦のターン進行（一人用モード。あなたが先攻で交互に手番。手番プレイヤーのコマが出目ぶん移動し、先に1周ゴールした方が勝ち。`GameFlowController` が統括）
- すごろくのループ盤面（外周マスを手番プレイヤーのコマが出目ぶん移動。コマは各プレイヤーのキャラの丸アイコン画像で描画（YOU＝選択キャラ、CPU＝人間と別のキャラをランダム選択、未配置は色＋YOU/CPU ラベルにフォールバック）。1周してゴール到達で勝ち）
