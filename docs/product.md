...
- マス着地の演出（コマが止まると、そのマスの画像を画面中央にカードで拡大表示。お金マスでは増減額「+ $n／- $n」を画像から上へ浮かび上がらせ〔増額は緑・減額は赤〕、画像とテキストが同じタイミングで消える。既定は画像 0.5 秒→テキスト 1.5 秒浮遊で計 2 秒。**陣地マスは専用の旗演出**＝そのコマのキャラの旗を画面中央に 1 秒表示してから縮小しながら対象マスへ重ね、占拠を確定〔以降そのマスは旗画像のまま〕。実装は `BoardPresenter.PlayLandingSequenceAsync`）→ [Assets/Scripts/Main/Board/](../Assets/Scripts/Main/Board/)
