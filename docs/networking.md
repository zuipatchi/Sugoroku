...
| `CellMessage` | 着地したマスで見せる文言のプール内 index（負値＝文言なし） | **ホスト** |
...
| `MiniGameReady` | （なし＝「はじめる」を押した） | 参加者全員（各自 1 通） |
...
**決める人が席に紐づかないものはホストが決める**: 着地したマスの文言（`CellMessage`）は「誰が着地したか」と関係なく 1 人が引けば足りる抽選なので、**着地した人ではなくホスト**が決めて配る（`OnlineGameSync.IsHost`。一人用モードは自分がホスト扱いなので同じ経路を通る）。見た目だけの値でも、同じマスに止まったのに席ごとに違う文言が出ると同じ画面を見ている感じが崩れるため配る。文字列ではなく**プール内 index** を送るのは、全クライアントが同じ資産（`BoardCellMessageCatalog`）を持っているから。**この待ちは着地演出のいちばん先頭に置く**（下の「先に来るはずのアクションを先に消費しておく」と同じ理由＝着地した人が配る `MoneyLanding` などより必ず先に流れるようにする）。**配ったのに誰も受け取らないアクションを残さない**ため、演出ごとスキップする判定（すでに自分が占拠している陣地マス）は文言を配ってもらう前に済ませる（判定材料は全員が同じ値なので結論も一致する）。

**送らないもの**: コマ移動・陣地占拠・勝敗判定は「誰が何マス進むか」と盤面データから決定論的に導けるので配らない。ルーレットの停止セクターも、`(進む人, 出目)` との割り当てが 1 対 1（`RouletteMath.ParticipantForSector` / `RouletteNumberLayout.StepsForSector`）なので **1 つの整数だけ**で足りる。出目の割り当てはスピンごとの抽選だが、これも配らずに済ませている（「部屋の全員で同じ値になるセッション ID」と「スピン回数」を種にして各クライアントが同じ表を組む＝`RouletteNumberLayout`。全員が同じアクション列で同じ回数だけ手番ループを回るのでスピン回数もそろう）。
...
### 全員でいっせいに始める操作は「1 人 1 通」を数えて待つ

ミニゲームマスに着地すると全員が同じゲームを同時に遊ぶので、**準備できていない人を置き去りにして始めてはいけない**（シーンが切り替わってから何のゲームか気付くことになる）。そこで告知の間に「はじめる」ボタンを出し、押した人が `MiniGameReady` を 1 通配って、**全席ぶん受信した時点で全クライアントが起動する**（`BoardPresenter.WaitForAllMiniGameReadyAsync`）。数えるのは受信したぶんだけなので、判定役を置かなくても起動のタイミングがそろう（`MiniGameScore` を集めるのと同じ形）。

一人用モードは CPU がボタンを押せないので、**自分が押した時点で全席ぶんを配る**（`OnMiniGameStartClicked`）。集計側は分岐せずそのまま使える。

**この待ちを足すときは、先に来るはずのアクションを先に消費しておく**。`WaitForActionAsync` は期待しない種別を警告して読み飛ばすので、`MiniGameReady` を待っている最中に `MiniGameLanding`（種）が届くと捨ててしまい、そのあとの種待ちが永久にハングする。順番を「種を受け取る → 告知して全員の `MiniGameReady` を待つ → 起動」にして交差させない。
