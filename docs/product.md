...
- すごろくのループ盤面（盤面データ `BoardDefinition` を読んで外周マスのループ盤面を描画し、手番プレイヤーのコマをルーレットの出目ぶん移動。周回勝利は廃止したのでスタート＝ゴールを通過して回り続ける。各マスはイベント（進む/戻る/休み/ミニゲーム・お金アップ/ダウン・陣地）と見た目（色）を持ち、マスの画像はイベント種別ごとに盤面が持つ（全マス共通の枠画像も重ねられる）。ただしスタート＝ゴール（先頭マス）は固定で `Board/Start` の画像を使う。お金マスと陣地マスが着地で発動。**盤面は左下の虫眼鏡 +／− ボタンで表示列数を段階ズーム**でき〔既定4列・拡大で3→2列・縮小で6→8列〕、画面外へはみ出したぶんはドラッグでパンして見る〔横長マップ向け・`BoardZoomController`〕。描画・レイアウト・ネームプレートの詳細は [CLAUDE.md](../CLAUDE.md)「主要ファイルの場所」BoardPresenter の行）→ [Assets/Scripts/Main/Board/](../Assets/Scripts/Main/Board/)
...
- カード取得（カード取得マス〔イベント「Card」〕に止まると `CardCatalog` からランダムに1枚もらえる。取得したカードは画面右下に手札としてサムネイル表示〔自分＝人間プレイヤーのぶんのみ。CPU は内部的に貯まるが非表示〕。カードは現状「集めて表示するだけ」で、**使う（効果発動）のは未実装**。カード種別・画像は `CardId`／`CardCatalog`、手札の保持は `CardModel`。カード画像は Addressables〔`Image/Card/*`〕、未配置ならカード名の文字で代替）→ [Assets/Scripts/Main/Card/](../Assets/Scripts/Main/Card/)
- マス着地の演出（コマが止まると、そのマスの画像を画面中央にカードで拡大表示。お金マスでは増減額「+ $n／- $n」を画像から上へ浮かび上がらせ〔増額は緑・減額は赤〕、画像とテキストが同じタイミングで消える。既定はお金マスが画像 0.5 秒→テキスト 1.5 秒浮遊で計 2 秒、それ以外のマス（スタート等）は画像を計 1 秒表示。**陣地マスは専用の旗演出**＝そのコマのキャラの旗を画面中央に 1 秒表示してから縮小しながら対象マスへ重ね、占拠を確定〔以降そのマスは旗画像のまま〕。**カード取得マスは抽選したカード絵を中央に約1秒見せてから右下の手札へ加える**。実装は `BoardPresenter.PlayLandingSequenceAsync`）→ [Assets/Scripts/Main/Board/](../Assets/Scripts/Main/Board/)
...
- 盤面マスのイベント発動（お金アップ/ダウン・陣地・カード取得は着地で発動する。進む/戻る/休み/ミニゲームは `BoardDefinition` で編集・盤面に記号表示できるが、止まったときに実際に発動させる処理は未実装。発動には `GameFlowController` / `TurnModel` への組み込みが必要）
- カードの使用（効果発動）（現状はカード取得マスで集めて右下に表示するだけ。取得したカードを使って相手の陣地/所持金を奪う・ミニゲームを起こすなどの効果は未実装。`CardModel` に手札を保持済みなので、使用 UI と効果処理を足して連携する）
