...
namespace Main.Board
{
    ...
    /// オンラインではキャラ選択ロビーで確定した席順ロースター（<see cref="OnlineRosterSessionModel"/>）を
    /// 全席に使う（＝全プレイヤーが被らないキャラで描画される）。
    ...
    public sealed class CpuCharacterPicker
    {
        ...
        {
            // オンラインはロビーで確定した席順ロースターをそのまま使う（全席・被らない）。
            if (_gameSession.Mode == GameMode.Online && _roster.HasRoster && player < _roster.Count)
            {
                return _roster.CharacterAt(player);
            }
