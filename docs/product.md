...
- CPU 対戦のターン進行（一人用モード。あなたが先攻で以降交互。人間の番は手動でルーレット、CPU の番は同じ円盤が自動で回る。ルーレットが消えてから手番プレイヤーのコマが出目ぶん進む。**勝敗は陣地マスの占拠数で決まる**（下記）。`GameFlowController` が統括し、オンラインは参加者 1 人で従来どおり単独プレイ）→ [Assets/Scripts/Main/Turn/](../Assets/Scripts/Main/Turn/)
- すごろくのループ盤面（盤面データ `BoardDefinition` を読んで外周マスのループ盤面を描画し、手番プレイヤーのコマをルーレットの出目ぶん移動。周回勝利は廃止したのでスタート＝ゴールを通過して回り続ける。各マスはイベント（進む/戻る/休み/ミニゲーム・お金アップ/ダウン・陣地）と見た目（色）を持ち、マスの画像はイベント種別ごとに盤面が持つ（全マス共通の枠画像も重ねられる）。ただしスタート＝ゴール（先頭マス）は固定で `Board/Start` の画像を使う。お金マスと陣地マスが着地で発動。描画・レイアウト・ネームプレートの詳細は [CLAUDE.md](../CLAUDE.md)「主要ファイルの場所」BoardPresenter の行）→ [Assets/Scripts/Main/Board/](../Assets/Scripts/Main/Board/)
...
- マス着地の演出（コマが止まると、そのマスの画像を画面中央にカードで拡大表示。お金マスでは増減額「+ $n／- $n」を画像から上へ浮かび上がらせ〔増額は緑・減額は赤〕、画像とテキストが同じタイミングで消える。既定はお金マスが画像 0.5 秒→テキスト 1.5 秒浮遊で計 2 秒、それ以外のマス（スタート等）は画像を計 1 秒表示。**陣地マスは専用の旗演出**＝そのコマのキャラの旗を画面中央に 1 秒表示してから縮小しながら対象マスへ重ね、占拠を確定〔以降そのマスは旗画像のまま〕。実装は `BoardPresenter.PlayLandingSequenceAsync`）→ [Assets/Scripts/Main/Board/](../Assets/Scripts/Main/Board/)
