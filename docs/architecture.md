...
Common (常駐)
  ...
  ├── GameSessionModel
  ├── CharacterSessionModel / OnlineRosterSessionModel / BoardSessionModel / PlayerCountSessionModel
...
- **ゲームを抜けるときは必ずセッションを離脱する**。NGO の寿命がシーンから切り離されたので、離脱しないとルームに残ったままになる。`BoardPresenter.ReturnHomeAsync`（「ホームに戻る」）・`OptionPresenter.BackToTitleAsync`（オプションの「タイトルへ戻る」。全シーンから押せるのでマッチング中・キャラ選択ロビー・対戦中のどこからでも通る）・`GameSessionModel.SetSinglePlayer`（一人用モードの選択）が離脱を担う
...
CommonLifeTimeScope   全シーン共通のシングルトンを登録
  ...
  ├── OnlineRosterSessionModel
  ├── BoardSessionModel
  ├── PlayerCountSessionModel
...
スクリプトは 9 つのランタイム Assembly Definition と、1 つのエディタ専用アセンブリに分割されている。

| アセンブリ | パス | 依存 |
|---|---|---|
| `Common` | `Assets/Scripts/Common/` | VContainer / R3 / UniTask / DOTween |
| `Title` | `Assets/Scripts/Title/` | VContainer / UniTask / Common |
| `Home` | `Assets/Scripts/Home/` | VContainer / UniTask / Addressables / Common |
| `CharacterSelect` | `Assets/Scripts/CharacterSelect/` | VContainer / R3 / UniTask / Addressables / Common |
| `MapSelect` | `Assets/Scripts/MapSelect/` | VContainer / R3 / UniTask / Addressables / Common / **Main**（`BoardCatalog` / `BoardDefinition` / `MapPickerView` を参照するため） |
| `Matching` | `Assets/Scripts/Matching/` | VContainer / R3 / UniTask / Addressables / Common / Unity.Services.Multiplayer / Unity.Netcode / **Main**（ルーム作成のマップ選択オーバーレイで `BoardCatalog` / `MapPickerView` を参照するため） |
...
| `Main` | `Assets/Scripts/Main/` | VContainer / R3 / UniTask / Addressables / Common / Unity.Services.Multiplayer / Unity.Netcode / DOTween |
| `MiniGame` | `Assets/Scripts/MiniGame/` | VContainer / R3 / UniTask / Addressables / Common / **Main**（被っちゃやーよが提示アイテムに `ItemCatalog` を参照するため） |
...
- `BoardSessionModel`（選択マップの識別子）は `Common` に置き文字列 ID だけを持つ。`BoardCatalog` / `BoardDefinition` / `MapPickerView` は `Main` にあるため、それらを扱う `MapSelect`・`Matching`（ルーム作成のマップ選択）と、`ItemCatalog` を使う `MiniGame`（被っちゃやーよ）が `Common` に加えて `Main` にも依存する
...
Assets/AddressableAssets/
  ├── Icon/        SVG アイコン
  ├── Image/
  │     ├── Background/  画面全体に敷く背景の絵
  │     ├── Board/       マスの絵（イベント種別ごと・Board/<イベント名>）
  │     ├── Character/   キャラのカード絵/コマ用バッジ/立ち絵/走行絵/旗絵（Character<N>/Card・/Icon・/Portrait・/Run・/Flag）
  │     ├── Icon/        盤面 UI のアイコン（コイン・占領地）
  │     ├── Item/        アイテム絵
  │     └── MiniGame/    ミニゲームの中身の絵（サムネイル・サンドバッグ等）
