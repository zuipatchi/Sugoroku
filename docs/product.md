...
- 所持金（お金）（プレイヤーごとに所持金を持つ。初期 1000・マイナス＝借金も可。お金アップ/ダウンのマスに止まると増減し、盤面上部の自分ネームプレートにコイン＋金額で表示・マイナスは赤字。金額は盤面エディタでマスごとに設定。ミニゲームアイテムでミニゲームに勝ったときの報酬〔既定 +500〕でも増える）→ [Assets/Scripts/Main/Money/](../Assets/Scripts/Main/Money/)
